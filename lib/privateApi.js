...
//  const stamp=new Date().toISOString().split(".")[0];
  const stamp=Date.now().toFixed(0);

//  var source=stamp+options.method+options.url+(options.body==undefined?'':options.body);
//  var source=stamp+options.method+options.url.replace(this.endPoint,'')+(Object.keys(options.body)==0?'':JSON.stringify(options.body));

//  var query=Object.assign({},options.body,options.qs);
...
//  var signature = encodeURIComponent( crypto.createHmac('sha256', this.secret).update(source).digest('base64') );
...
KucoinPrivate.prototype.getOrderbook = function(symbol) { // https://docs.kucoin.com/#get-full-order-book-aggregated
...
KucoinPrivate.prototype.getAccountLedgers = function(options) { // https://docs.kucoin.com/#get-account-ledgers
  const path="/api/v1/accounts/ledgers";
  return this.getQuery(path, options);
};
...
KucoinPrivate.prototype.getTransferable = function(options) { // https://docs.kucoin.com/#get-the-transferable
  const path="/api/v1/accounts/transferable";
  return this.getQuery(path, options);
};
...
KucoinPrivate.prototype.getDepositAddress = function(currency) { // https://docs.kucoin.com/#get-deposit-addresses-v2
  const path="/api/v2/deposit-addresses?currency="+currency;
  return this.getQuery(path, {});
};
...
KucoinPrivate.prototype.getHistDeposits = function(options) { // https://docs.kucoin.com/#get-v1-historical-deposits-list
  const path="/api/v1/hist-deposits";
  return this.getQuery(path, options);
};
...
KucoinPrivate.prototype.getHistWithdrawals = function(options) { // https://docs.kucoin.com/#get-v1-historical-withdrawals-list
  const path="/api/v1/hist-withdrawals";
  return this.getQuery(path, options);
};
...
KucoinPrivate.prototype.getUserFee = function(type) { // https://docs.kucoin.com/#basic-user-fee
  var path="/api/v1/base-fee";
  if(type!==undefined) { path+="?currencyType="+type; };
  return this.getQuery(path, {});
};

KucoinPrivate.prototype.getSymbolFee = function(symbols) { // https://docs.kucoin.com/#actual-fee-rate-of-the-trading-pair
  const path="/api/v1/trade-fees?symbols="+symbols;
  return this.getQuery(path, {});
};
...
KucoinPrivate.prototype.getSpotClientOrderID = function(id,symbol) { // https://docs.kucoin.com/#get-single-order-by-clientoid
  var path="/api/v1/stop-order/queryOrderByClientOid/"+id;
  if(symbol!==undefined) { path+="?symbol="+symbol; };
  return this.getQuery(path, {});
};

KucoinPrivate.prototype.cancelSpotClientOrderID = function(id,symbol) { // https://docs.kucoin.com/#cancel-single-order-by-clientoid-2
  var path="/api/v1/stop-order/cancelOrderByClientOid/"+id;
  if(symbol!==undefined) { path+="?symbol="+symbol; };
  return this.otherQuery("DELETE", path, {});
};
...
KucoinPrivate.prototype.getMarginRiskLimit = function(model="cross") { // https://docs.kucoin.com/#query-the-cross-isolated-margin-risk-limit
  const path="/api/v1/risk/limit/strategy?marginModel="+model;
  return this.getQuery(path, {});
};
...
KucoinPrivate.prototype.getRepayRecord = function(currency) { // https://docs.kucoin.com/#get-repay-record
  var path="/api/v1/margin/borrow/outstanding";
  if(currency!==undefined) { path+="?currency="+currency; };
  return this.getQuery(path, {});
};

KucoinPrivate.prototype.getRepaymentRecord = function(currency) { // https://docs.kucoin.com/#get-repayment-record
  var path="/api/v1/margin/borrow/repaid";
  if(currency!==undefined) { path+="?currency="+currency; };
  return this.getQuery(path, {});
};
...
KucoinPrivate.prototype.getMarginOrders = function(currency) { // https://docs.kucoin.com/#get-active-order
  var path="/api/v1/margin/lend/active";
  if(currency!==undefined) { path+="?currency="+currency; };
  return this.getQuery(path, {});
};

KucoinPrivate.prototype.getLendHistory = function(options) { // https://docs.kucoin.com/#get-lent-history
  const path="/api/v1/margin/lend/done";
  return this.getQuery(path, options);
};
...
KucoinPrivate.prototype.getSettledLendOrders = function(options) { // https://docs.kucoin.com/#get-settled-lend-order-history
  const path="/api/v1/margin/lend/trade/settled";
  return this.getQuery(path, options);
};

KucoinPrivate.prototype.getAccountLendRecord = function(currency) { // https://docs.kucoin.com/#get-account-lend-record
  var path="/api/v1/margin/lend/assets";
  if(currency!==undefined) { path+="?currency="+currency; };
  return this.getQuery(path, {});
};

KucoinPrivate.prototype.getLendingMarketData = function(currency,term) { // https://docs.kucoin.com/#lending-market-data
  var path="/api/v1/margin/market?currency="+currency;
  if(term!==undefined) { path+="&term="+term; };
  return this.getQuery(path, {});
};

KucoinPrivate.prototype.getMarginTradeData = function(currency) { // https://docs.kucoin.com/#margin-trade-data
  const path="/api/v1/margin/trade/last?currency="+currency;
  return this.getQuery(path, {});
};
...
KucoinPrivate.prototype.getIsolatedAccounts = function(currency) { // https://docs.kucoin.com/#query-isolated-margin-account-info
  var path="/api/v1/isolated/accounts";
  if(currency!==undefined) { path+="?balanceCurrency="+currency; };
  return this.getQuery(path, {});
};
