...
//  privateUrl = 'wss://push1-v2.kucoin.com/endpoint?token=MYTOKEN&connectId=MYCONNECT';
...
//        if(this.compressed) {
//          message = JSON.parse(pako.inflate(msg.data,{to:'string'}));
//        } else { 
...
//        };
        ...
        switch(message.type) {
          ...
          case "welcome":
            console.log("WELCOME",this.name,message.id);
            break;

          case "ack":
            console.log("ACK",message,this._promises);
            if (this._promises.has(message.id)) {
              const cb = this._promises.get(message.id);
              this._promises.delete(message.id);
console.log("PR",this._promises);
...
console.log("PROM",this._promises,options);
...
module.exports = {
  publicApi: function(keys,token) { return new KuCoinSocket(publicUrl, undefined, token, GZIP); },
