...
KuCoinSocket.prototype.subscribeTicker = async function(symbols=["all"]) { // https://docs.kucoin.com/#symbol-ticker
  ...
  const key="/market/ticker:"+symbols.join();
...
KuCoinSocket.prototype.unsubscribeTicker = async function(symbols=["all"]) { // https://docs.kucoin.com/#symbol-ticker
                                                                           // https://docs.kucoin.com/#all-symbols-ticker
  const key="/market/ticker:"+symbols.join();
  const reqID="id"+(++this.socket._id);
  const options={id:reqID,type:"unsubscribe",topic:key,response:true};
  const result = await this.socket.request(reqID,options);
  return result;
};
...
KuCoinSocket.prototype.unsubscribeSnapshot = async function(symmarket) { // https://docs.kucoin.com/#symbol-snapshot
                                                                       // https://docs.kucoin.com/#market-snapshot
  const key="/market/snapshot:"+symmarket;
  const reqID="id"+(++this.socket._id);
  const options={id:reqID,type:"unsubscribe",topic:key,response:true};
...
KuCoinSocket.prototype.subscribeOrderbook = async function(symbols) { // https://docs.kucoin.com/#level-2-market-data
  const key="/market/level2:"+symbols.join();
  const reqID="id"+(++this.socket._id);
  const options={id:reqID,type:"subscribe",topic:key,response:true};
  const result = await this.socket.request(reqID,options);
  return result;
...
KuCoinSocket.prototype.unsubscribeOrderbook = async function(symbols) { // https://docs.kucoin.com/#level-2-market-data
  const key="/market/level2:"+symbols.join();
  const reqID="id"+(++this.socket._id);
  const options={id:reqID,type:"unsubscribe",topic:key,response:true};
  const result = await this.socket.request(reqID,options);
...
KuCoinSocket.prototype.subscribeBest = async function(symbols,depth) { // https://docs.kucoin.com/#level2-5-best-ask-bid-orders
  ...
  const key="/spotMarket/level2Depth"+depth.toFixed(0)+":"+symbols.join(); // depth is 5 or 50
  const reqID="id"+(++this.socket._id);
  const options={id:reqID,type:"subscribe",topic:key,response:true};
  const result = await this.socket.request(reqID,options);
  return result;
...
KuCoinSocket.prototype.unsubscribeCandles = async function(symbol,period) { // https://docs.kucoin.com/#klines
  const key="/market/candles:"+symbol+"_"+period;
  const reqID="id"+(++this.socket._id);
  const options={id:reqID,type:"unsubscribe",topic:key,response:true};
  const result = await this.socket.request(reqID,options);
...
KuCoinSocket.prototype.subscribeTrades = async function(symbols,private=false) { // https://docs.kucoin.com/#match-execution-data
  const key="/market/match:"+symbols.join();
  const reqID="id"+(++this.socket._id);
...
KuCoinSocket.prototype.unsubscribeTrades = async function(symbols,private=false) { // https://docs.kucoin.com/#match-execution-data
  const key="/market/match:"+symbols.join();
  const reqID="id"+(++this.socket._id);
  const options={id:reqID,type:"unsubscribe",topic:key,response:true,privateChannel:private};
  const result = await this.socket.request(reqID,options);
  return result;
};

KuCoinSocket.prototype.subscribeIndexPrice = async function(symbols) { // https://docs.kucoin.com/#index-price
  const key="/indicator/index:"+symbols.join();
  const reqID="id"+(++this.socket._id);
  const options={id:reqID,type:"subscribe",topic:key,response:true};
  const result = await this.socket.request(reqID,options);
  return result;
...
KuCoinSocket.prototype.unsubscribeIndexPrice = async function(symbols) { // https://docs.kucoin.com/#index-price
  const key="/indicator/index:"+symbols.join();
  const reqID="id"+(++this.socket._id);
  const options={id:reqID,type:"unsubscribe",topic:key,response:true};
  const result = await this.socket.request(reqID,options);
...
KuCoinSocket.prototype.subscribeMarkPrice = async function(symbols) { // https://docs.kucoin.com/#mark-price
  const key="/indicator/markPrice:"+symbols.join();
  const reqID="id"+(++this.socket._id);
  const options={id:reqID,type:"subscribe",topic:key,response:true};
  const result = await this.socket.request(reqID,options);
  return result;
...
KuCoinSocket.prototype.unsubscribeMarkPrice = async function(symbols) { // https://docs.kucoin.com/#mark-price
  const key="/indicator/markPrice:"+symbols.join();
  const reqID="id"+(++this.socket._id);
  const options={id:reqID,type:"unsubscribe",topic:key,response:true};
  const result = await this.socket.request(reqID,options);
...
KuCoinSocket.prototype.unsubscribeMarginBook = async function(currencies) { // https://docs.kucoin.com/#order-book-change
  const key="/margn/fundingBook:"+currencies;
  const reqID="id"+(++this.socket._id);
  const options={id:reqID,type:"unsubscribe",topic:key,response:true};
  const result = await this.socket.request(reqID,options);
...
KuCoinSocket.prototype.subscribeOrderUpdates = async function() { // https://docs.kucoin.com/#private-order-change-events
  const key="/spotMarket/tradeOrdersV2";
  const reqID="id"+(++this.socket._id);
...
KuCoinSocket.prototype.unsubscribeOrderUpdates = async function() { // https://docs.kucoin.com/#private-order-change-events
  const key="/spotMarket/tradeOrdersV2";
  const reqID="id"+(++this.socket._id);
  const options={id:reqID,type:"unsubscribe",topic:key,response:true,privateChannel:true};
  const result = await this.socket.request(reqID,options);
  return result;
};
...
KuCoinSocket.prototype.unsubscribeBalanceUpdates = async function() { // https://docs.kucoin.com/#account-balance-notice
  const key="/account/balance";
  const reqID="id"+(++this.socket._id);
  const options={id:reqID,type:"unsubscribe",topic:key,response:true,privateChannel:true};
  const result = await this.socket.request(reqID,options);
...
KuCoinSocket.prototype.subscribeDebtRatioUpdates = async function() { // https://docs.kucoin.com/#debt-ratio-change
...
KuCoinSocket.prototype.unsubscribeDebtRatioUpdates = async function() { // https://docs.kucoin.com/#debt-ratio-change
                                                                       // https://docs.kucoin.com/#position-status-change-event
  const key="/margin/position";
  const reqID="id"+(++this.socket._id);
  const options={id:reqID,type:"unsubscribe",topic:key,response:true,privateChannel:true};
...
KuCoinSocket.prototype.unsubscribeMarginOrders = async function(currency) { // https://docs.kucoin.com/#margin-trade-order-enters-event
                                                                            // https://docs.kucoin.com/#margin-order-update-event
                                                                            // https://docs.kucoin.com/#margin-order-done-event
  const key="/margin/load:"+currency;
  const reqID="id"+(++this.socket._id);
...
KuCoinSocket.prototype.unsubscribeStopOrders = async function() { // https://docs.kucoin.com/#stop-order-event
  const key="//spotMarket/advancedOrders";
  const reqID="id"+(++this.socket._id);
  const options={id:reqID,type:"unsubscribe",topic:key,response:true,privateChannel:true};
  const result = await this.socket.request(reqID,options);
